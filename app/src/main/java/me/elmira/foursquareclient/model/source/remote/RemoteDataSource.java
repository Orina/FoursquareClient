...
import me.elmira.foursquareclient.model.source.remote.data.Item;
import me.elmira.foursquareclient.model.source.remote.data.Photos;
    ...
    public void searchVenues(String latLng, int radius, int limit, final SearchVenuesCallback callback) {
        ...
        responseCall.enqueue(new Callback<ExploreResponse>() {
            ...
            public void onResponse(Call<ExploreResponse> call, Response<ExploreResponse> response) {
                ...
                List<me.elmira.foursquareclient.model.Venue> resultVenues = new ArrayList<>();
                for (Item item : searchResponse.getResponse().getGroups().get(0).getItems()) {
                    resultVenues.add(new me.elmira.foursquareclient.model.Venue(item.getVenue(), item.getTips()));
                }
